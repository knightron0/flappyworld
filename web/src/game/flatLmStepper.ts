...
export class FlatLMStepper {
  ...
  private checkpointIds: number[] | null = null
  ...
  contextTokens(): { ids: number[]; positions: number[] } {
    const tokens = ['<bos>']
    ...
    let positions = ids.map((_, index) => index)
  ...
  frameTokensForContext(frame: FrameTokens, present: [boolean, boolean]): string[] {
    ...
    tokens.push(
      ...
      `done_${frame.done}`,
      `action_${frame.action}`,
    )
    return tokens
  }

  private async syncContext(ids: number[], positions: number[]): Promise<Float32Array> {
    return this.engine.syncContext(this.checkpointIds, ids, positions)
  }
  ...
  private async appendToken(ids: number[], positions: number[], token: string): Promise<Float32Array> {
    const nextPos = positions.length > 0 ? positions[positions.length - 1] + 1 : 0
    const tokenId = requireTokenId(this.vocab, token)
    ids.push(tokenId)
    positions.push(nextPos)
    return this.forwardToken(tokenId, nextPos)
  }
  ...
  private async sampleToken(
    ...
    ids.push(tokenId)
    positions.push(nextPos)
    const nextLogits = await this.forwardToken(tokenId, nextPos)
  ...
  async step(flap: boolean): Promise<StepResult> {
    ...
    const { ids, positions } = this.contextTokens()
    const generatedTokens: string[] = []

    let logits = await this.syncContext(ids, positions)

    const [birdYToken, birdLogits] = await this.sampleToken(ids, positions, logits)
    const birdY = parseValueToken(birdYToken)
    generatedTokens.push(birdYToken)
    logits = birdLogits

    const [pipe0PresentToken, pipe0PresentLogits] = await this.sampleToken(ids, positions, logits)
    ...
    if (pipe0Present) {
      const [pipe0XToken, pipe0XLogits] = await this.sampleToken(ids, positions, logits)
      ...
      const [pipe0GapToken, pipe0GapLogits] = await this.sampleToken(ids, positions, pipe0XLogits)
      ...
      pipe0Respawn = false
      logits = await this.appendToken(ids, positions, 'pipe0_x_hidden')
      logits = await this.appendToken(ids, positions, 'pipe0_gap_hidden')
    ...
    const [pipe1PresentToken, pipe1PresentLogits] = await this.sampleToken(ids, positions, logits)
    generatedTokens.push(pipe1PresentToken)
    const pipe1Present = parseValueToken(pipe1PresentToken) === 1
    logits = pipe1PresentLogits
    if (pipe1Present) {
      const [pipe1XToken, pipe1XLogits] = await this.sampleToken(ids, positions, logits)
      ...
      const [pipe1GapToken, pipe1GapLogits] = await this.sampleToken(ids, positions, pipe1XLogits)
      ...
      pipe1Respawn = false
      logits = await this.appendToken(ids, positions, 'pipe1_x_hidden')
      logits = await this.appendToken(ids, positions, 'pipe1_gap_hidden')
      generatedTokens.push('pipe1_x_hidden', 'pipe1_gap_hidden')
    }

    const [respawnToken, respawnLogits] = await this.sampleToken(ids, positions, logits)
    const [doneToken] = await this.sampleToken(ids, positions, respawnLogits)
    generatedTokens.push(respawnToken, doneToken)
    await this.appendToken(ids, positions, `action_${action}`)
    ...
    const cacheSeqLen = this.engine.cacheSeqLen
    this.checkpointIds = cacheSeqLen > 0 ? ids.slice(-cacheSeqLen) : null
    this.lastTimings = {
      ...inference,
